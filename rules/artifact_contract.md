...
7. **用户审批文档默认中文**：凡需要用户阅读、评审或明确批准的文档型 Artifact，标题、章节名、正文、表格说明、风险、结论、建议与审批说明必须尽量使用中文。代码、接口/字段名、文件路径、固定状态枚举（如 `USER_REVIEW`）、协议/库/产品专有名词可保留英文；如英文术语会影响理解，应优先给出中文含义。只有用户明确要求其他语言时才切换。所有面向审批的文档模板也必须遵守本规则。
...
- 规则/规格/设计：优先 Markdown，便于版本控制和 Agent 阅读；其中需要用户审批的文档默认以中文为主要表达语言。
